...
dump_live_process_backtraces() {
  ...
  local pids
  pids=$(ps -eo pid=,comm= | awk '$2 == "eloqdoc" || $2 == "dss_server" || $2 == "launch_sv" || $2 == "host_manager" {print $1}' | head -12)

  if [ -z "${pids}" ]; then
    echo "No live eloqdoc, dss_server, launch_sv, or host_manager processes found."
    return 0
  fi
...
find_binary_for_core() {
  local base="$1"
  local root exe candidate
  shift
  ...
  for root in "$@"; do
    ...
    exe=$(find "${root}" -maxdepth 6 -type f -name "${base}" 2>/dev/null | head -1)
    if [ -n "${exe}" ]; then
      echo "${exe}"
      return 0
    fi
  ...
  # Only guess a known binary when gdb could not identify the crashing program.
  if [ -n "${base}" ] && [ "${base}" != "unknown" ]; then
    return 1
  fi

  for candidate in eloqdoc dss_server launch_sv host_manager; do
    for root in "$@"; do
      if [ ! -d "${root}" ]; then
        continue
      fi
      exe=$(find "${root}" -maxdepth 6 -type f -name "${candidate}" 2>/dev/null | head -1)
      if [ -n "${exe}" ]; then
        echo "${exe}"
        return 0
      fi
...
dump_core_backtraces() {
  ...
  local cores
  cores=$(find /tmp /var/crash "${ELOQDOC_BASE_PATH:-/nonexistent}" "${ELOQDOC_INSTALL_PREFIX:-/nonexistent}" -maxdepth 4 -type f \
  ...
  local -a binary_roots=(
    "${ELOQDOC_INSTALL_PREFIX:-/nonexistent}"
    "${ELOQDOC_BASE_PATH:-/nonexistent}"
  )

  local core prog base exe
  for core in ${cores}; do
    ...
    exe=$(find_binary_for_core "${base}" "${binary_roots[@]}")
