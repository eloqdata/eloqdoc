...
#   4. Builds EloqDoc (CMake Eloq module + SCons MongoDB server under Python 3)
#
...
_log() { printf '\033[1;34m[eloqdoc]\033[0m %s\n' "$*" >&2; }
...
if ! _dep_done data_substrate; then
    ...
    ELOQDB_ELOQ_MODULE=ELOQ_MODULE_ELOQDOC bash "$ELOQDB_ROOT/scripts/substrate.sh"
fi

export ELOQDOC_DIR="$SCRIPT_DIR"
exec bash "$ELOQDB_ROOT/scripts/projects/eloqdoc.sh"
