...
def idl_scanner(node, env, path):
    ...
    cmd = [sys.executable, "scripts/buildscripts/idl/idlc.py",  '--include','src', str(node), '--write-dependencies']
    ...
    nodes_deps_list.extend(env.Glob('#scripts/buildscripts/idl/*.py'))
    nodes_deps_list.extend(env.Glob('#scripts/buildscripts/idl/idl/*.py'))
...
def generate(env):
    ...
    env['IDLC'] = sys.executable + " scripts/buildscripts/idl/idlc.py"
    env['IDLCFLAGS'] = ''
