...
Status EloqRecordStore::_insertRecords(OperationContext* opCtx,
    ...
    const std::unordered_map<uint16_t, std::pair<txservice::TableName, txservice::SecondaryKeySchema>>* allIndexes = tableSchemaToUse->GetIndexes();
    if (allIndexes && !allIndexes->empty()) {
        // Collect unique indexes
        std::vector<std::pair<txservice::TableName, const Eloq::MongoKeySchema*>> uniqueIndexes;
        for (const auto& [kid, index] : *allIndexes) {
            const auto* keySchema = static_cast<const Eloq::MongoKeySchema*>(index.second.sk_schema_.get());
            if (keySchema && keySchema->Unique()) {
                uniqueIndexes.emplace_back(index.first, keySchema);
            }
        }

        // If there are unique indexes, validate all records using batchGetKV
        if (!uniqueIndexes.empty()) {
            // For each unique index, collect all keys to check
            for (const auto& [indexName, keySchema] : uniqueIndexes) {
                ...
                // std::vector<std::pair<size_t, BSONObj>> recordKeyMapping;  // Maps batch index to (record index, BSON key)

                // For each document to be inserted
                for (size_t i = 0; i < nRecords; i++) {
                    BSONObj obj{records[i].data.data()};
                    ...
                    for (const BSONObj& key : keys) {
                        ...
                        // keyString.resetToKey(key, keySchema->Ordering(), records[i].id);
                        ...
                        // recordKeyMapping.emplace_back(i, key.getOwned());

                        MONGO_LOG(0) << "yf: mongo key = " << mongoKeys.back()->ToString() << ", key = " << key.toString();

                        // Add to batch (mongoRecord is not needed, just pass nullptr or empty record)
                ...
                if (!indexBatchTuples.empty()) {
                    MONGO_LOG(0) << "yf: index name = " << indexName.StringView() << ", n records = " << nRecords << ", check unique index";
                    // Use batchGetKV to check all keys
                    txservice::TxErrorCode err = ru->batchGetKV(
                        opCtx, indexName, keySchema->SchemaTs(), indexBatchTuples, true);
                    if (err != txservice::TxErrorCode::NO_ERROR) {
                        MONGO_LOG(1) << "EloqRecordStore::_insertRecords batchGetKV failed for unique index, index: "
                                     << indexName.StringView() << ", error: " << txservice::TxErrorMessage(err);
                        return TxErrorCodeToMongoStatus(err);
                    }
                    ...
                    for (size_t batchIdx = 0; batchIdx < indexBatchTuples.size(); batchIdx++) {
                        ...
                        if (tuple.status_ == txservice::RecordStatus::Normal) {
                            ...
                            MONGO_LOG(0) << "yf: duplicate key found, index: " << indexName.StringView() << ", key: " << keyStringBuffers[batchIdx];
