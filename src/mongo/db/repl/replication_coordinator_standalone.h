...
/**
 * MongoDB standalone coordination only. Distributed transactions, replication and durability
 * belong to the Eloq storage engine; this class must never start MongoDB replica-set workers.
 */
class ReplicationCoordinatorStandalone final : public repl::ReplicationCoordinator {

public:
    explicit ReplicationCoordinatorStandalone(ServiceContext* serviceContext);
    ~ReplicationCoordinatorStandalone() override;

    ReplicationCoordinatorStandalone(const ReplicationCoordinatorStandalone&) = delete;
    ReplicationCoordinatorStandalone& operator=(const ReplicationCoordinatorStandalone&) = delete;

    // Standalone implementations of the shared ReplicationCoordinator interface.
    ...
    // Replica-set operations reject requests; local server operations have modeNone semantics.
...
private:
    ...
    // Preserve the server parameter's set/get behavior without starting a prefetch worker.
    std::atomic<ReplSettings::IndexPrefetchConfig> _indexPrefetchConfig{
